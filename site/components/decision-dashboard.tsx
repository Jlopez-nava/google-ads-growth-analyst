...
export function DecisionDashboard({ data }: { data: DashboardData }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f6fa] text-[#242932]">
      ...
      <footer className="border-t border-[#dde1e8] bg-white"><div className="mx-auto flex max-w-[1500px] flex-col gap-2 px-5 py-6 text-xs text-[#8d949f] sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12"><p>{data.account.currency} · {data.account.timeZone} · data through {data.period.end}</p><p>Fictional portfolio snapshot · recommendations only</p></div></footer>
    </main>
  );
}
